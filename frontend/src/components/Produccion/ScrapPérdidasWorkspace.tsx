...
export const ScrapPérdidasWorkspace: React.FC<ScrapPérdidasWorkspaceProps> = ({
  ...
  const filteredEvents = useMemo(() => {
    return scrapEvents.filter((ev) => {
      ...
      return matchProc && match4M;
    });
  }, [scrapEvents, selectedProcess, selectedCategory4M]);
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        {/* Unidades Totales de Scrap */}
        <div className="rounded-2xl border border-theme-subtle bg-theme-surface p-4">
          <small className="text-[10px] font-bold uppercase text-theme-muted block">Unidades Scrap Hoy</small>
          <div className="flex items-baseline gap-2 mt-1">
            <b className="font-mono text-3xl font-black text-theme-main">
              {formatNumber(totalScrapUnits || 6142)}
            </b>
            <span className="text-[11px] text-theme-muted">ejs</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border border-theme-subtle bg-theme-surface p-5 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { process: 'Flexografía Rotativa (Mark Andy Scout)', pct: 4.2, status: 'warning', note: 'Mayor merma en arranques y cambios de suaje' },
              { process: 'Offset Plano (Heidelberg SM 74)', pct: 3.1, status: 'normal', note: 'Pruebas de entintado CMYK en pliegos iniciales' },
              { process: 'Doblado y Acabados (Stahlfolder Ti-52)', pct: 2.2, status: 'good', note: 'Calibración de bolsas de plegado' },
              { process: 'Grapado en Lomo / Encuadernación', pct: 1.6, status: 'good', note: 'Desperdicio de alambre y recorte trilateral' },
              { process: 'Rebobinado e Inspección (Rotoflex)', pct: 1.3, status: 'good', note: 'Descarte por empalmes de bobina matriz' },
            ].map((item) => (
              <div key={item.process} className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="font-bold text-theme-main">{item.process}</span>
                  <b
                    className={`font-mono ${
            ...
            {[
              {
                ...
                details: 'Ajuste de registro, tensión de bobina, rodillo desgastado',
              },
              {
                category: '4M Método',
                pct: 28,
                ...
                details: 'Merma de arranque y pruebas de color, velocidad inadecuada',
              },
              {
                ...
                details: 'Variación espesor sustrato, tono fuera de pantone',
              },
              {
                ...
                details: 'Manipulación de pliego, error de montaje de suaje',
              },
            ].map((item) => (
      ...
      <div className="rounded-2xl border border-theme-subtle bg-theme-surface p-5 space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-theme-muted">
              Reportes directos ingresados por los operadores en las terminales táctiles de máquina.
            </p>
          </div>
          ...
          <table className="w-full text-xs">
            <thead className="bg-theme-muted/40 text-[10px] uppercase text-theme-muted">
              <tr>
                ...
                <th className="p-2.5 text-left">Causa Técnica / Acción</th>
              ...
              {filteredEvents.map((ev) => (
                <tr key={ev.id} className="hover:bg-theme-muted/20">
                  ...
                  <td className="p-2.5 text-[11px]">
                    ...
                    <small className="text-theme-muted">{ev.comment}</small>
                  </td>
